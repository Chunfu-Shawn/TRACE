...
class Trainer:
    ...
    def _cache_expression_means(self):
        """Compute and cache cross-species cell-type-specific mean expression vectors
        from the pre-built global_expr_dict."""
    ...
    def count_task_criterion(
            ...
            count_embs_padded: torch.Tensor,
        ...
        count_embs_padded = count_embs_padded.float()
        ...
        weighted_masks = pad_masks.float() * token_weights
        ...
        # ==========================================
        if pred.shape[2] == 1:
            loss_all = self.dynamics_criterion(pred.squeeze(-1), count_embs_padded.squeeze(-1)) 
            loss_all = loss_all * pad_masks.float() * token_weights
        else:
            loss_all = self.dynamics_criterion(pred, count_embs_padded) 
            loss_all = loss_all * pad_masks.unsqueeze(-1).float() * token_weights.unsqueeze(-1)
        ...
        if pred.shape[2] == 1:
            ...
            t_val = count_embs_padded.squeeze(-1)
        else:
            p_val = pred.sum(dim=-1)
            t_val = count_embs_padded.sum(dim=-1)
            
        # ---------------------------------------------------------
        # Calculate overall CDS Mean for Ranking Loss
        # ---------------------------------------------------------
        cds_eval_mask = pad_masks.to(device) & cds_masks
        ...
        for i, f_mask in enumerate(frame_masks):
            target_eval_mask = pad_masks.to(device) & f_mask
    ...
    def train_epoch(self, epoch: int) -> Tuple[torch.Tensor, List]:
        ...
        for batch_idx, batch_data in enumerate(dataloader):
            # Unpack the batch
            _, species_list, _, cell_mask, expr_batch, \
