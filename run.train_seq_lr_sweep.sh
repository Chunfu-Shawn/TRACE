...
# requested GPU, so CUDA_VISIBLE_DEVICES should not be overwritten manually.
...
export TRACE_ALPHA_START="0"
export TRACE_ALPHA_FINAL="0"
...
for learning_rate in 0.005 0.001 0.0001; do
    ...
    torchrun --standalone --nproc-per-node=1 "${PROJECT_DIR}/run.train_seq.py" \
        > "${train_log}" 2>&1
